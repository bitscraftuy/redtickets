	 		...
	 		echo '<nav id="navigation_bar" class="navigation '.$add_menu_class.'">'; //open nav holder
				...
				if( get_option( RT_THEMESLUG."_show_search_menu" ) ):?>
					...
					<div class="search-bar">
						<form action="<?php echo esc_url(rt_wpml_get_home_url()); ?>" method="get" class="showtextback" id="menu_search">
							<fieldset>							
								<input type="text" class="search_text showtextback" name="s" id="menu_search_field" value="<?php _e('Search','rt_theme');?>" />		
								<div class="icon-search-1"></div>					
							</fieldset>
							<?php if( defined( "ICL_LANGUAGE_CODE" ) ) : ?><input type="hidden" name="lang" value="<?php echo esc_html(ICL_LANGUAGE_CODE); ?>"/><?php endif;?>
						</form>
					</div>
					<!-- / search-->
				<?php endif;?> 
