					...
					<ul id="top_navigation" class="top_links">
						...
						<?php if(!get_option(RT_THEMESLUG.'_hide_woo_cart') && class_exists( 'Woocommerce' )):?>
							...
							<?php if ( is_user_logged_in() ) { ?>
								<li class="icon-user"><a href="<?php echo esc_url(get_permalink( rt_wpml_translated_page_id(get_option('woocommerce_myaccount_page_id')) )); ?>" title="<?php _e('My Account','rt_theme'); ?>"><?php _e('My Account','rt_theme'); ?></a></li>
								<li class="icon-logout"><a href="<?php echo esc_url(wp_logout_url(home_url('/'))); ?>" title="<?php _e('Logout','rt_theme'); ?>"><?php _e('Logout','rt_theme'); ?></a></li>
							<?php } else { ?>
								<li class="icon-login"><a href="<?php echo esc_url(get_permalink( get_option('woocommerce_myaccount_page_id') )); ?>" title="<?php _e('Login / Register','rt_theme'); ?>"><?php _e('Login / Register','rt_theme'); ?></a></li>
							<?php } ?>						  			

							<li class="icon-basket"><span><a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php _e('View your shopping cart', 'rt_theme'); ?>"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'rt_theme'), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a></span></li>					
						<?php endif;?>
	 

						<?php if( get_option( RT_THEMESLUG."_show_search_top" ) ):?>
							<li><form action="<?php echo rt_wpml_get_home_url(); ?>/" method="get" id="top_search_form"><span class="icon-search"></span><span><input type="text" class="search_text showtextback" size="1" name="s" id="top_search_field" placeholder="<?php _e('Search','rt_theme');?>" /></span><?php if( defined( "ICL_LANGUAGE_CODE" ) ) : ?><input type="hidden" name="lang" value="<?php echo esc_html(ICL_LANGUAGE_CODE); ?>"/><?php endif;?></form></li>
						<?php endif;?>
