...
function rt_create_fonts_css(){
	...
	$selected_fonts = array( 
		"menu"    => get_option( RT_THEMESLUG.'_fonts_menu' ), 
		"heading" => get_option( RT_THEMESLUG.'_fonts_heading' ), 
		"body"    => get_option( RT_THEMESLUG.'_font_body' ), 
		"serif"   => get_option( RT_THEMESLUG.'_fonts_serif' )
	);
...
function create_rt_color_css( $color_set = array(), $container = "" ){
		...
		$css .= ! empty( $link ) ? sprintf('
					...
					%2$s .woocommerce-MyAccount-navigation > ul > li > a:hover, 
					%2$s .woocommerce-MyAccount-navigation > ul > li.is-active > a 
					{
						color: %1$s;
					}
		',
		...
		$css .= ! empty( $border ) ? sprintf('
					...
					.woocommerce %2$s div.product div.images,
					.woocommerce %2$s .woocommerce-MyAccount-navigation > ul,
					.woocommerce %2$s .woocommerce-MyAccount-navigation > ul > li:not(:last-child) > a,					
					%2$s table,
					...
					%2$s .woocommerce-MyAccount-navigation > ul,
					%2$s .woocommerce-MyAccount-navigation > ul > li:not(:last-child) > a					
					{
					border-color:%1$s;		
					}
