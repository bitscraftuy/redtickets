...
if( ! function_exists("rt_global_variables") ){
	function rt_global_variables( $templates='' ){
		...
		if ( class_exists( 'Woocommerce' ) ) {		 
			$woo_page_id ="";
			$woo_page_id = (is_product_category() || is_shop()) ? wc_get_page_id('shop') : $woo_page_id;
...
if( ! function_exists("get_resized_image_output") ){
	function get_resized_image_output( $atts = false ){
		...
		if ( empty( $image_id ) && empty( $image_url ) ){
			...
			//conpatibility fix for versions before 2.9
			$image_id = ! empty( $image_url ) && is_numeric( $image_url ) && empty( $image_id ) ? $image_url : $image_id;
...
if( ! function_exists("rt_get_image_data") ){
	function rt_get_image_data($args){
		...
		//conpatibility fix for versions before 2.9
		$image_id = ! empty( $image_url ) && is_numeric( $image_url ) && empty( $image_id ) ? $image_url : $image_id;
		$image_url = ! empty( $image_url ) && is_numeric( $image_url ) && empty( $image_id ) ? wp_get_attachment_url( $image_url ) : $image_url;
		...
		if( $attachment ){
			...
			$orginal_image_url = empty( $image_url ) || is_numeric( $image_url ) ? wp_get_attachment_url( $image_id ) : $image_url ;
...
add_action('layerslider_ready', 'rt_layerslider_set_as_theme');
function rt_layerslider_set_as_theme(){
	if(function_exists("layerslider_set_as_theme")){
		layerslider_set_as_theme();
	}
}
add_action('layerslider_ready', 'rt_layerslider_hide_promotions');
function rt_layerslider_hide_promotions(){	
...
add_action( 'wp_ajax_nopriv_listar_talleres', 'listarTalleres' );
add_action( 'wp_ajax_listar_talleres', 'listarTalleres' );

function listarTalleres()
{
	global $wpdb;
	$queryStr = "SELECT * FROM wp_ebp_talleres" ;
	$talleres = $wpdb->get_results($queryStr, OBJECT);

	echo json_encode($talleres);
	exit();
}

add_action( 'wp_ajax_nopriv_update_cupo', 'updateCupoTalleres' );
add_action( 'wp_ajax_update_cupo', 'updateCupoTalleres' );


function updateCupoTalleres(){
			...
			switch($obj->event){
				case 3: //1 ticket 1 dia
					...
					for($i = 0; $i < count($talleres);$i++){
						...
						$cupo = ($current == 0) ? 0 : (int)($current-1);
						$newCupo = (string)$cupo;

						$current2 = (int)$talleres[$i]['cupo2'];
						$cupo2 = ($current2 == 0) ? 0 : (int)($current2-1);
