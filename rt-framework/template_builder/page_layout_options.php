...
class RTThemePageLayoutOptions extends RTThemeAdmin{
	...
	function rt_import_page_templates( $file_content = "", $oldurl = "" ){
		global $pageLayoutClass;
		...
		//demo image urls
		$find_replace = apply_filters("rt18_import_replace",array (
			'{{demo_image}}' => $demo_image_url
		)); 

		//replace the old url 
		if( ! empty( $oldurl ) ){
			$find_replace[$oldurl] = get_site_url(); 
		}

		$file_content = strtr($file_content, $find_replace);
		...
		foreach($template_contents as $item_num => $template){ 
			...
			elseif( $content_type == "portfolio_carousel" ){
			 	...
			 	*/
			 	...
			 	$display_descriptions = $display_titles = $crop = "";
